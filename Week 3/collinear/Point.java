...
public class Point implements Comparable<Point> {
    ...
    public double slopeTo(Point that) {
        if (this.x - that.x == 0) {
            if (this.y - that.y == 0) {
                // Invoking point and argument point are the same point.
    ...
    public int compareTo(Point that) {
        if ((this.y == that.y) && (this.x == that.x)) {
            // Invoking point is equal to argument point.
            return 0;
        }

        if (this.y < that.y) {
            // Invoking point is 'less' than argument point.
            return -1;
        }
        ...
        // If we reach this point, then invoking point must be 'greater' than argument point.
        return 1;
    }
    ...
    public Comparator<Point> slopeOrder() {
        return new Comparator<Point>() {
            public int compare(Point p1, Point p2) {
                ...
                /*
                // code replaced by Double/compare():
                if (slope1 == slope2) {
                    return 0;
                }
